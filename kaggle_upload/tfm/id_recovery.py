...
# Verbosidad de los logs internos del EDA.
#   False (por defecto) -> silencioso: NO imprime ni la traza por iteración del
#          optimizador ni los mensajes por run. El grid_search ya muestra el
#          progreso útil (Repetición 1/5, 2/5...), así que en Kaggle no
#          queremos inundar el stdout con miles de líneas.
#   True  -> modo depuración en local (iteraciones, meta de fitness, parada...).
VERBOSE = False
...
class IDRecovery:
    ...
    def run(self, g=100, i=100, target_fitness=1e-5, patience=10, min_delta=1e-4):
        ...
        self.target_fitness = self._compute_target(len(self.train_rules))
        if VERBOSE and self.fitness_type in ['softmax', 'entropy']:
        ...
        try:
            if VERBOSE:
                ...
                # Silencia cualquier traza por iteración que EDAspy escriba en
                # stdout (p.ej. "IT: n  Best cost: ..."), pase lo que pase con
                # el flag disp. El fitness no imprime, así que es seguro.
